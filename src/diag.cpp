...
diag_line calc(float slope, float &x, float& y)
{
    float p1x = static_cast<int>(y + 1) / slope;
    float p1y = slope * static_cast<int>(x + 1);

    if (static_cast<int>(x + 1) < p1x) p1x = p1y / slope;
    if (static_cast<int>(y + 1) < p1y) p1y = slope * p1x;
...
{
    constexpr int WH = PIXEL_SIZE * N_SPLIT;
    int floored_x = std::floor(line.p0.x);
    int floored_y = std::floor(line.p0.y);

    return {
        { (line.p0.x - floored_x) * WH, (line.p0.y - floored_y) * WH },
        { (line.p1.x - floored_x) * WH, (line.p1.y - floored_y) * WH }
    };
}
...
int fmain()
{
    ...
    for (int i = 0; i < N; i++)
    {
        results[i] = calc(2.f/5, x, y);
        std::cout << results[i] << std::endl;
    }


    std::cout << "Using diag_calcer:" << std::endl;
    diag_calcer dc(5, 2);

    auto list = dc.calc_line();
